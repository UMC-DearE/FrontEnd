...
export default function ProfileCard({ nickname, bio, imgUrl, onClickSettings }: ProfileCardProps) {
  return (
    <div className="relative flex min-h-[202px] w-full max-w-[440px] flex-col items-center rounded-[10px] bg-[#FAFAFA] shadow-[0_0_4px_0_rgba(0,0,0,0.12)]">
      <button
        type="button"
        onClick={onClickSettings}
        className="absolute right-4 top-4 cursor-pointer"
      >
        <img src={setIcon} alt="set-icon" className="h-[32px] w-[32px]" />
      </button>
      ...
      <p className="mt-[10px] h-[14px] text-[14px] font-medium text-[#9D9D9F]">
        {bio || '소개를 입력하세요'}
      </p>
    </div>
  );
