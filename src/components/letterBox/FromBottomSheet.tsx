        ...
        <div className="fixed bottom-0 left-1/2 w-[393px] h-[260px] -translate-x-1/2 rounded-t-[17px] bg-white">
          <div className="flex h-full flex-col">
            ...
            <div className="mt-[14px] flex-1 overflow-y-auto px-[24px] pb-4 no-scrollbar">
              {/* 전체 보기 */}
              <div
                className={`mx-auto rounded-[8px] transition-all ${
                  selectedId === 'all' ? 'bg-[#FFEEE8]' : 'bg-white'
                }`}
              >
                <button
                  type="button"
                ...
                >
                  <p
                    className={`text-[16px] ${
                      selectedId === 'all' ? 'text-[#FF5F2F] font-semibold' : 'text-[#141517]'
                    }`}
                  >
                    모든 편지 보기
                  </p>
                  <p
                    className={`text-[16px] ${
                      selectedId === 'all' ? 'text-[#FF5F2F] font-bold' : 'text-[#45454599]'
                    }`}
                  >
                    {totalCount}
                  </p>
                ...
                {froms.map((from) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex h-[44px] w-full items-center justify-between rounded-[8px] px-3 transition-all ${
                        isSelected ? 'bg-[#FFEEE8]' : 'bg-white'
                      }`}
                    >
                      <p
                        className={`text-[16px] ${
                          isSelected ? 'text-[#FF5F2F] font-semibold' : 'text-[#141517]'
                        }`}
                      >
                        {from.name}
                      </p>
                      <p
                        className={`text-[16px] ${
                          isSelected ? 'text-[#FF5F2F] font-bold' : 'text-[#45454599]'
                        }`}
                      >
